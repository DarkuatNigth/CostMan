    ...
    public class ProcesoParametroController : ControllerBase
    {
        ...
        public async Task<IActionResult> ParamProc(string strAnio, string strMes)
        {
            ...
            try
            {
                ...
                lstCostProd = await _objOperacionComercial.ObtenerInformacionCostProd(Convert.ToInt16(strAnio), Convert.ToInt16(strMes));
                WarrenResultadoDto? objWarren = await _objCostoMateriaPrima.ConsultarWarrenPeriodo(
                    Convert.ToInt16(strAnio),
                    Convert.ToInt16(strMes));
                ...
                var dtResult = new DataTablesResultDto
                {
                    ...
                    {
                        WarrenPeriodoResumenDto.Desde(objWarren)
                    }.AListaDeDiccionarios(),
                    Table8 = (objWarren?.lstDetalle ?? new List<WarrenProcesoDto>())
                        .AListaDeDiccionarios()
                };
     ...
     [FromBody] GuardarCostoProductivoRequest objRequest)
            ...
            [FromBody] GuardarCierreParamProcRequest request)
        {
            ...
            {
                if (request == null ||
                    !int.TryParse(request.strAnio, out int anio) ||
                    !int.TryParse(request.strMes, out int mes) ||
                ...
                var tareaCosto = _objOperacionComercial.ObtenerCostoProductivo(anio, mes);
                var tareaData = _objCostoMateriaPrima.ObtenerParametroProceso(fecha);

                await Task.WhenAll(tareaCosto, tareaData);

                CostoProductivoResultadoDto costo = await tareaCosto;
                DataProcesoParam data = await tareaData;

                List<ProcesoResultadoDto> parametros =
                    CierreParamProcBuilder.ConstruirParametros(
                        data,
                        costo.lstProcesosOrigen);

                // Warren se calcula contra el costo productivo recién construido,
                // nunca contra el snapshot PFR del período anterior.
                WarrenResultadoDto warren = MotorWarrenCierre.Calcular(
                    costo.lstProcesosOrigen,
                    data.lstProcesoFrs,
                    request.dcObjetivoWarren);

                CierreParamProcResultadoDto resultado =
                    await _objOperacionComercial.GuardarCierreParamProc(
                        anio,
                        mes,
                        request.strUsuario,
                        costo,
                ...
                {
                    blStatus = true,
                    strMensaje = "Cierre ParamProc guardado correctamente.",
                    objData = resultado
                });
            }
            catch (Exception ex)
