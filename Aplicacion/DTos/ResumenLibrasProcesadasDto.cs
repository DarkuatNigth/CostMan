...
namespace CostManagementService.Aplicacion.DTos
{
    ...
    public static class ParamProcVistaBuilder
    {
        ...
        public static ParamProcVistaResultadoDto Construir(DataProcesoParam objData)
        {
            ...
            var lstRpc = objData.lstLiqRepro ?? new List<MatPrimaReproceso>();
            ...
            resultado.lstResumen = ConstruirResumen(lstFrs, lstRpcProcesado);
        ...
        {
            var frs = new Dictionary<string, decimal>(StringComparer.OrdinalIgnoreCase);
            foreach (var item in lstFrs)
                AcumularParticion(frs, ParticionCosteo.ClasificarFrs(item.strProClas01, item.strProClas05), (decimal)item.dcLibras);

            var rpc = new Dictionary<string, decimal>(StringComparer.OrdinalIgnoreCase);
            foreach (var item in lstRpcProcesado)
                AcumularParticion(rpc, ParticionCosteo.ClasificarRpc(item.strTipoProducto), (decimal)item.dbLibras);

            return new List<ResumenLibrasProcesadasDto>
            {
                CrearResumen("PFR", "Camarón Libras liquidadas", frs, 1, incluirBase: true, incluirVa: true),
                CrearResumen("RPC", "Camarón Reproceso", rpc, 2, incluirBase: true, incluirVa: false),
                CrearResumen("VAG", "Camarón VAG", rpc, 3, incluirBase: false, incluirVa: true)
            };
        }
            ...
            bool incluirVa)
        {
            return new ResumenLibrasProcesadasDto
            {
                ...
                dcLibrasEntero = incluirBase ? dic.GetValueOrDefault(ParticionCosteo.ENTERO, 0m) : 0m,
                dcLibrasCola = incluirBase ? dic.GetValueOrDefault(ParticionCosteo.COLA, 0m) : 0m,
                dcLibrasValorAgregado = incluirVa
                    ? dic.GetValueOrDefault(ParticionCosteo.ENTERO_VA, 0m) +
                      dic.GetValueOrDefault(ParticionCosteo.COLA_VA, 0m)
                    : 0m,
                intOrden = orden
            };
        }
