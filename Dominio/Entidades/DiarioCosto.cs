...
namespace CostManagement.Aplicación.DTos
{
    public class DiarioCosto
    {
        ...
        [JsonIgnore]
        public string? strDescri { get; set; }
        ...
        public void InitializeKeys()
        {
            ...
            this.objLoteProdTalReciKey = new LoteRpcKeyReci(this.intLote, Convert.ToInt32(this.strProCodcor), (int)this.stTalCodigo);
        }
        ...
        public void InicializarCamposCost(decimal dbLidPrecio)
        {
            dcCostoUnit = dbLidPrecio;
            dcCostoTot = dbLidPrecio * this.dcLibras;
        }
    }
    ...
    public class InventarioVal
    {
        ...
        [NotMapped]
        [JsonIgnore]
        public LoteRpcKeyXProdTal objLoteProdTalKey { get; set; }
        ...
        public void InicializarCampos(InfoProd objProd, string TalDescri)
        {
            ...
            strCongel = objProd.strDprDescri ?? "IQF";
            ...
            objLoteProdTalReciKey = new LoteRpcKeyReci(intLote, Convert.ToInt32(strProCodcor), (int)stTalCodigo);
            objLotkey = new LoteFrsKey(intLote, Convert.ToInt32(strProCodcor), (int)stTalCodigo);
            objLoteProdTalKey = new LoteRpcKeyXProdTal(Convert.ToInt32(strProCodcor), stTalCodigo);
            strTalDescri = TalDescri;
        }

        public static Dictionary<LoteRpcKeyXProdTal, decimal> GenerarDiccionarioProdXTalla(List<InventarioVal> lstCostVentUni)
        {
            if (!lstCostVentUni.Any()) return new Dictionary<LoteRpcKeyXProdTal, decimal>();
            return new Dictionary<LoteRpcKeyXProdTal, decimal>(
                lstCostVentUni
                //.Where(x =>/* x.strTipo == "E" &&*/ _lstTipCod.Contains(x.strCodTip) && x.dcLibras > 0)
                .GroupBy(x => x.objLoteProdTalKey)
                .ToDictionary(
                    g => g.Key,
        ...
        public static Dictionary<LoteFrsKey, decimal> GenerarDiccionarioCostoXTalla(
            List<InventarioVal> lstCostVentUni)
        {
            try
            {
                if (!lstCostVentUni.Any()) return new Dictionary<LoteFrsKey, decimal>();

                var dicSinKey = new Dictionary<(int intLote, string strProCodcor, short stTalCodigo), InventarioVal>();
                var lstConKey = new List<InventarioVal>();

                foreach (var obj in lstCostVentUni)
                {
                    if (obj.objLotkey is null)
                    {
                        var key = (obj.intLote, obj.strProCodcor, obj.stTalCodigo);
                        dicSinKey[key] = obj;   // last-write-wins si hay claves repetidas
                    }
                    else
                    {
                        lstConKey.Add(obj);
                ...
                if (dicSinKey.Any())
                {
                    Console.WriteLine($"[GenerarDiccionarioCostoXTalla] Registros SIN objLotkey (no costeados): {dicSinKey.Count}");
                    foreach (var kvp in dicSinKey)
                    {
                        var obj = kvp.Value;
                        Console.WriteLine(
                            $"  -> Lote={obj.intLote} | ProCod='{obj.strProCodcor}' | Talla={obj.stTalCodigo} " +
                            $"| CodTip='{obj.strCodTip}' | Libras={obj.dcLibras} | CostoUnit={obj.dcCostoUnit}");
                    }
                }
            ...
            {
                Console.WriteLine($"Error al generar diccionario de costo por talla: {objExcep.Message}");
                throw new Exception($"Error al generar diccionario de costo por talla: {objExcep.Message}", objExcep);
            }
        }
    }
    ...
    public class CostVentUni
    {
        ...
        public CostVentUni(
    string strTipoLiq, string strTipo, int? intLoteUni, int intLote,
    string strCodProd, string strDescripcion, string strTalla, int intCodTalla,
    string strProcProd, string strAgrupacion, string strTipoProd,
    string strCongelamiento, string strClase, decimal dcLibras)
        {
            this.strTipoLiq = strTipoLiq;
            this.strTipo = strTipo;
            this.intLoteUni = intLoteUni;
            this.intLote = intLote;
            this.intCodProd = Convert.ToInt32(strCodProd);
            this.strDescripcion = strDescripcion;
            this.strTalla = strTalla;
            this.intCodTalla = intCodTalla;
            this.strProcProd = strProcProd;
            this.strAgrupacion = strAgrupacion;
            this.strTipoProd = strTipoProd;
            this.strCongelamiento = strCongelamiento;
            this.strClase = strClase;
            this.dcLibras = dcLibras;
            this.dcCostoUnit = 0;
            this.dcCostoTot = 0;
            objLoteProdTalKey = new LoteFrsKey(intLote, this.intCodProd, intCodTalla);
        }
    ...
    public class InfoTalProd
    {
        ...
        [Column("codTallaMP")]
        public int? intCodTallaMp { get; set; }

        [Column("descTallaMP")]
        public string? strDescTallaMp { get; set; }

        [Column("codTallaVTA")]
        public int? intCodTallaVta { get; set; }

        [Column("descTallaVTA")]
        public string? strDescTallaVta { get; set; }
